  ...
  return (
    <aside className="w-full xl:w-64 glass-panel border-y-0 border-l-0 rounded-none xl:rounded-r-2xl xl:h-screen flex flex-col xl:sticky xl:top-0 shrink-0 z-50">
      <div className="p-4 xl:p-6 border-b border-white/10 flex justify-between items-center relative overflow-hidden">
        <div className="absolute top-0 right-0 w-32 h-32 bg-brandGold/5 rounded-full blur-3xl"></div>
        <div className="flex items-center space-x-3 relative z-10">
          ...
          <h1 className="text-lg font-bold text-slate-200 tracking-widest uppercase">Ironwood<span className="text-brandGold">SEO</span></h1>
        </div>
      ...
      <div className={`${isOpen ? 'block' : 'hidden'} xl:flex flex-col flex-grow p-4 space-y-2`}>
        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3 ml-2 hidden xl:block">Command Module</p>
        {navItems.map(item => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-300 relative overflow-hidden ${isActive ? 'text-white bg-slate-700/50 shadow-sm' : 'text-slate-400 hover:bg-slate-700/30 hover:text-slate-200'}`}
            >
